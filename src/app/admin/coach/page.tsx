...
import CoachForm from '@/src/components/forms/AnggotaForm'
...
const CoachPage = () => {
    ...
    const handleAddCoach = async (data: any): Promise<void> => {
        try {
    ...
    const handleEditCoach = async (data: any) => {
        try {
            ...
            if (Object.keys(userData).length > 0) {
                ...
                if (result.success) {
                    await loadUsers() // Refresh the list
                    console.log('Pelatih berhasil diperbarui')
                } else {
                    console.error('Error updating pelatih:', result.error)
                }
            } else {
    ...
    const handleDeleteCoach = async (userId: number) => {
        try {
    ...
    const handleExportExcel = async () => {
        try {
            setExporting(true)
            if (!users.length) {
                alert('Tidak ada data pelatih untuk diekspor')
                return
            }
            ...
            await exportToXlsx('pelatih', rows)
        } catch (error) {
            console.error('Error exporting users:', error)
            alert('Gagal mengekspor data pelatih')
        } finally {
            setExporting(false)
        }
    }
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <CoachForm
                ...
                onSubmit={editingData ? handleEditCoach : handleAddCoach}
                        ...
                        <div className="p-4">
                            ...
                            <div className="flex gap-2">
                               ...
                               >
                                ...
                                <button
                                   onClick={() => handleDeleteCoach(user.unique_id)}
